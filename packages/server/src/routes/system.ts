...
export async function searchExpression(
    ...
    const startNum = (Math.floor(Date.now() / 1000) % 2) * 48;
    ...
    const res = await axios({
        ...
        url: `https://pic.sogou.com/napi/wap/pic?query=${encodeURIComponent(`${keywords}`)}+%E8%A1%A8%E6%83%85&start=${startNum}&reqFrom=wap_result`,
        headers: {
            accept: '*/*',
    ...
    //     img_arr.push({image:element.thumbSrc,width:100,height:100});
    // })
    data.forEach((element) => {
        img_arr.push({ image: element.locImageLink, width: 100, height: 100 });
    });

    img_arr.sort(() => Math.random() - 0.5);
